...
describe('UnshortenUrlResult Component', () => {
  test('renders correctly with given result data', () => {
    render(<UnshortenUrlResult result={mockResult} resetUrlInput={mockResetUrlInput} />);
    ...
    expect(screen.getByRole('button', { name: /Unshorten another URL/i })).toBeInTheDocument();
  });

  test('calls resetUrlInput function when button is clicked', () => {
    render(<UnshortenUrlResult result={mockResult} resetUrlInput={mockResetUrlInput} />);
    
    fireEvent.click(screen.getByRole('button', { name: /Unshorten another URL/i }));
