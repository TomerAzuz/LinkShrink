...
describe('UrlReportResult Component', () => {
  test('renders success message when report is successful', () => {
    render(<UrlReportResult result={mockResultSuccess} resetUrlInput={mockResetUrlInput} />);
    
    expect(screen.getByText('Report submitted successfully ✅')).toBeInTheDocument();
  });

  test('renders failure message when report fails', () => {
    render(<UrlReportResult result={mockResultFail} resetUrlInput={mockResetUrlInput} />);
  ...
  test('calls resetUrlInput function when button is clicked', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Report another URL/i }));
